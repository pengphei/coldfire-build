...
int  do_nand_boot_update(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong mem_addr, data_size;
	int j;
	struct mtd_info *mtd;
	u_char *addr;
	u32 saved_cfg;
	...
	data_size = simple_strtoul(argv[2], NULL, 16);

	saved_cfg = nfc_read(mtd, NFC_FLASH_CONFIG);

	nfc_write(mtd, NFC_FLASH_CONFIG, 0x000ea671);
	memcpy(CONFIG_SYS_NAND_BASE, (unsigned char *)mem_addr, 0xf80);
	...
	if (nand_write_skip_bad(nand, 0x2000, &data_size, addr))
	{
